...
__attribute__((noinline)) void passThrough_aie(T *__restrict in, T *__restrict out,
                                               const int32_t height,
                                               const int32_t width) {
//   event0();

  aie::vector<T, N> *__restrict outPtr = (aie::vector<T, N> *)out;
  aie::vector<T, N> *__restrict inPtr  = (aie::vector<T, N> *)in;

  AIE_PREPARE_FOR_PIPELINING
  AIE_LOOP_MIN_ITERATION_COUNT(6)
  for (int j = 0; j < (height * width); j += N) {
    *outPtr++ = *inPtr++;
  }
